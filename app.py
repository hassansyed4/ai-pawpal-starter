...
This app helps pet owners manage pet care tasks, build a schedule, and detect simple conflicts.
"""
)
...
for their pet(s) based on time, task status, and recurring needs.
"""
    )
        ...
        task_table = [
            {
                ...
                "Due Time": item["task"].due_time,
                "Duration": item["task"].time_minutes,
                "Frequency": item["task"].frequency,
                "Completed": item["task"].completed,
            }
            for item in scheduler.sort_by_time(all_tasks)
        ]
        st.table(task_table)
    else:
